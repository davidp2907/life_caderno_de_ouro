...
function App() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Validate aplicador name if "assistida" is selected
    ...
    if (Object.keys(newErrors).length === 0) {
      // Set application date on submit
      const now = new Date();
      setFormData(prev => ({
        ...prev,
    ...
    <div className="min-h-screen bg-gradient-to-br from-cyan-50 to-teal-100 p-6">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="space-y-6">
              <h1 className="text-3xl font-semibold text-center text-teal-700 mb-2">Caderninho de ouro</h1>
              <div className="bg-teal-50 p-4 rounded-lg text-teal-800 space-y-4">
                <p>ESTE CADERNINHO DESTINA-SE A AJUDAR A ORGANIZAR E HIERARQUIZAR AS SUAS IDEIAS E RESPOSTAS ÀS TAREFAS APRESENTADAS. ATRAVÉS DESTE PROCESSO, PRETENDE-SE QUE REFLITA SOBRE O QUE É VERDADEIRAMENTE IMPORTANTE PARA SI E JUSTIFIQUE AS SUAS ESCOLHAS DE FORMA CLARA E CONCISA.</p>
              </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div className="flex gap-4">
                  ...
                  {formData.aplicador === 'assistida' && (
                    <input
                      ...
                      className={`w-full p-2 border rounded-lg ${errors.nomeAplicador ? 'border-red-500' : ''}`}
                      value={formData.nomeAplicador}
                      onChange={(e) => setFormData({ ...formData, nomeAplicador: e.target.value })}
                    />
                  )}
                </div>
                {errors.nomeAplicador && (
                ...
                <div className="space-y-2">
                  <label className="flex items-center">
                    <input
                      ...
                      onChange={(e) => setFormData({ ...formData, escolhaAtividade: e.target.value })}
                      className="mr-2"
                    />
                    A Própria Pessoa
                  </label>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) => setFormData({ ...formData, fractalComportamento: e.target.value })}
                />
              ...
              <table className="w-full border-collapse">
                <thead>
                  ...
                  <tr>
                    ...
                    <td className="border p-2 text-left text-sm text-gray-500">Releia as respostas da coluna (1), identifique qual a de maior importancia e escreva o número 3 nesta coluna, a de média importancia 2, e na outra coluna o número 1.</td>
                    <td className="border p-2 text-left text-sm text-gray-500">Justifique cada resposta da coluna (1). Escreva as suas justificativas nessa coluna no número corresposndente a resposta.</td>
                  </tr>
                </thead>
                <tbody>
                ...
                <option value="Média">2</option>
                <option value="Baixa">1</option>
                </select>
                </td>
                      <td className="border p-2">
                        <input
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-lg p-6 shadow-lg">
            ...
            <div className="space-y-6">
              <div>
                <label className="block text-teal-700 font-medium mb-2">
                  AGORA PARE, PENSE E ESCREVA COMO VOCÊ ESTA SE SENTINDO, OU SE SENTIU NO DESEMPENHO DESSA TAREFA
                </label>
                <textarea
                  className="w-full p-2 border rounded-lg h-32"
                  value={formData.feedbackFinal}
                  onChange={(e) => setFormData({ ...formData, feedbackFinal: e.target.value })}
                />
              </div>
            </div>
          </div>
